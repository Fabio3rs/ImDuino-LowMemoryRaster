...
template<typename POS, typename COL>
inline bary_t<POS, COL> baryPre(
    const pixel_t<POS, COL>& a,
...
{
    pixel_t<POS, COL> p2; p2.x = p.x - bary.a.x; p2.y = p.y - bary.a.y;
...
template<typename POS, typename COL>
inline void barycentricUVCol(pixel_t<POS, COL>& p, const bary_t<POS, COL>& bary)
{
    pixel_t<POS, COL> p2; p2.x = p.x - bary.a.x; p2.y = p.y - bary.a.y;
    POS d20 = dot(p2, bary.p0);
    POS d21 = dot(p2, bary.p1);
