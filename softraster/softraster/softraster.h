...
void renderQuadCore(SCREEN *screen, const texture_t<TEXTURE> &tex,
                    const clip_t<POS> &clip,
                    const rectangle_t<POS, COLOR> &quad, const bool alphaBlend,
                    POS currentX, POS currentY) {
        ...
        if (alphaBlend) {
            ...
            if (y >= ry.min && y < ry.max) {
                for (POS x = rx.min; x < rx.max; ++x) {
                    screen[x] %= quad.p1.c * tex.at(x + u, y + v);
                }
            }
        } else {
            // for (POS y = ry.min; y < ry.max; ++y)
...
void renderQuadCore(SCREEN *screen, const clip_t<POS> &clip,
                    const rectangle_t<POS, COLOR> &quad, const bool alphaBlend,
                    POS currentX, POS currentY) {
    if ((quad.p2.x < clip.x.min) || (quad.p2.y < clip.y.min) ||
        (quad.p1.x >= clip.x.max) || (quad.p1.y >= clip.y.max))
...
void renderQuad(SCREEN *screen, const texture_base_t *tex,
                const clip_t<POS> &clip, const rectangle_t<POS, COLOR> &quad,
                const bool alphaBlend, POS currentX, POS currentY) {
    switch (tex == nullptr ? texture_type_t::NONE : tex->type) {
    case texture_type_t::ALPHA8:
        renderQuadCore(screen, *reinterpret_cast<const texture_alpha8_t *>(tex),
                       clip, quad, alphaBlend, currentX, currentY);
        break;

    case texture_type_t::VALUE8:
        renderQuadCore(screen, *reinterpret_cast<const texture_value8_t *>(tex),
                       clip, quad, alphaBlend, currentX, currentY);
        break;

    case texture_type_t::COLOR16:
        renderQuadCore(screen,
                       *reinterpret_cast<const texture_color16_t *>(tex), clip,
                       quad, alphaBlend, currentX, currentY);
        break;

    case texture_type_t::COLOR24:
        renderQuadCore(screen,
                       *reinterpret_cast<const texture_color24_t *>(tex), clip,
                       quad, alphaBlend, currentX, currentY);
        break;

    case texture_type_t::COLOR32:
        renderQuadCore(screen,
                       *reinterpret_cast<const texture_color32_t *>(tex), clip,
                       quad, alphaBlend, currentX, currentY);
        break;

    default:
        renderQuadCore(screen, clip, quad, alphaBlend, currentX, currentY);
        break;
    }
}

template <typename POS, typename SCREEN, typename TEXTURE, typename COLOR>
void renderTriCore(SCREEN *screen, const texture_t<TEXTURE> &tex,
                   const clip_t<POS> &clip, const range_t<POS> &rY,
                   const range_t<POS> &rX1, const range_t<POS> &rX2,
                   const bary_t<POS, COLOR> &bary, const bool alphaBlend,
                   POS currentX, POS currentY) {
    POS y = currentY;
...
template <typename POS, typename SCREEN, typename COLOR>
void renderTriCore(SCREEN *screen, const clip_t<POS> &clip,
                   const range_t<POS> &rY, const range_t<POS> &rX1,
                   const range_t<POS> &rX2, const bary_t<POS, COLOR> &bary,
                   const bool uvBlend, const bool alphaBlend, POS currentX,
                   POS currentY) {
    POS y = currentY;
    ...
    if (uvBlend) {
        if (alphaBlend) {
            ...
            if (y >= ry.min && y < ry.max) {
                const float f = (y - rY.min) / (float)(rY.max - rY.min);
                const range_t<POS> rx = inl_min(
                    {lerp(rX1.min, rX2.min, f), lerp(rX1.max, rX2.max, f)},
                    clip.x);
                for (POS x = rx.min; x < rx.max; ++x) {
                    pixel_t<POS, COLOR> p;
                    p.x = x;
...
void renderTri(SCREEN *screen, const texture_base_t *tex,
    ...
    switch (tex == nullptr ? texture_type_t::NONE : tex->type) {
    case texture_type_t::ALPHA8:
        renderTriCore(screen, *reinterpret_cast<const texture_alpha8_t *>(tex),
                      clip, rY, rX1, rX2, bary, alphaBlend, currentX, currentY);
        break;

    case texture_type_t::VALUE8:
        renderTriCore(screen, *reinterpret_cast<const texture_value8_t *>(tex),
                      clip, rY, rX1, rX2, bary, alphaBlend, currentX, currentY);
        break;

    case texture_type_t::COLOR16:
        renderTriCore(screen, *reinterpret_cast<const texture_color16_t *>(tex),
                      clip, rY, rX1, rX2, bary, alphaBlend, currentX, currentY);
        break;

    case texture_type_t::COLOR24:
        renderTriCore(screen, *reinterpret_cast<const texture_color24_t *>(tex),
                      clip, rY, rX1, rX2, bary, alphaBlend, currentX, currentY);
        break;

    case texture_type_t::COLOR32:
        renderTriCore(screen, *reinterpret_cast<const texture_color32_t *>(tex),
                      clip, rY, rX1, rX2, bary, alphaBlend, currentX, currentY);
        break;
...
void renderTri(SCREEN *screen, const texture_base_t *tex,
    ...
    } else if (tri.p1.y == tri.p2.y) // Flat top triangle
    {
        ...
        renderTri(screen, tex, clip, {tri.p1.y, tri.p3.y}, {tri.p1.x, tri.p2.x},
                  {tri.p3.x, tri.p3.x}, baryPre(tri.p1, tri.p2, tri.p3),
                  uvBlend, alphaBlend, currentX, currentY);
    } else {
        ...
        renderTri(screen, tex, clip, {tri.p1.y, tri.p2.y}, {tri.p1.x, tri.p1.x},
                  {tri.p3.x, p4.x}, baryPre(tri.p1, tri.p2, p4), uvBlend,
                  alphaBlend, currentX, currentY);

        renderTri(screen, tex, clip, {tri.p2.y, tri.p3.y}, {tri.p2.x, p4.x},
                  {tri.p3.x, tri.p3.x}, baryPre(tri.p2, p4, tri.p3), uvBlend,
                  alphaBlend, currentX, currentY);
    }
}

template <typename POS, typename SCREEN>
void renderCommand(texture_t<SCREEN> &screen, SCREEN *Line,
                   const texture_base_t *texture, const ImDrawVert *vtx_buffer,
        ...
        for (int n = 0; n < drawData->CmdListsCount; n++) {
            ...
            for (int cmdi = 0; cmdi < cmdList->CmdBuffer.Size; cmdi++) {
                ...
                if (pcmd.UserCallback) {
                    ...
                    renderCommand<POS>(screen, Line,
                                       reinterpret_cast<const texture_base_t *>(
                                           pcmd.TextureId),
                                       vtx_buffer, idx_buffer, pcmd, 0, y);
                }
